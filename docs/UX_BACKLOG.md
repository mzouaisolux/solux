...
| 7 | Split task-list page + required-empty + mapping up front | ✅ | Sectioned + quick-nav (S1-6) ✅. **Sprint 3**: top **readiness banner** surfaces required-empty fields + missing mappings BEFORE the gate (`countRequiredEmpty` pure helper + `page.tsx` banner, unit-tested). Verified live on an under-validation list. |
| 8 | Mapping readiness at launch, not only at gate | ✅ | **Sprint 3**: missing-mapping count now shown in the top readiness banner (under_validation + validated), using the SAME pure helper as the release gate (no divergence). Per-row "Missing factory mapping" flags already inline. Verified live: under-validation list shows "5 factory mappings missing"; clean lists hide it. |
...
### Sprint 3 — Task Lists & Production Launch  ✅ COMPLETE (2026-06-30)
> **Delivered:** #7+#8 top **release-readiness banner** — surfaces required-empty fields (`countRequiredEmpty`, new pure helper, 5 unit tests) + missing factory mappings (same helper as the gate) on under_validation/validated lists, BEFORE the Release modal; #12 task-list domain verified English; S3-2 line saves now fire success/error toasts (consistent with the SaveButton) + gained error handling. **Files:** `lib/task-list-mapping-status.ts`, `task-lists/[id]/page.tsx`, `task-lists/[id]/tasklist.css`, `task-lists/[id]/TaskLineEditor.tsx`, `tests/task-list-mapping-status.test.ts`. **#20 skeletons deferred → Sprint 6.** **Verified** (genuine TLM login): under-validation list with 6 flagged rows shows "5 factory mappings missing"; lists with no blockers correctly hide the banner; validated list renders clean. `tsc` (no new errors) + `check:schema` ✅ + `npm test` 252/252 + `e2e:regression` 23/23. Committed.
...
### Sprint 4 — Operations  ✅ COMPLETE (2026-06-30)
> **Delivered:** #16 the two KPI strips merged into one section (Key numbers + Business snapshot rows, all 8 numbers, bottom strip removed); #15 delay chips made plain-language on BOTH surfaces — OrdersInFlight pills ("Nd late · factory/external") + action-center slip chips ("Baseline / Now due / Nd late"); D3 "Done" softened to a quiet neutral button (was an easy emerald mis-click; recoverable via existing `unacknowledgeAction`); O1 Operations pricing shown as an explicit sequence (intro "complete in order" + numbered ①②③ Cost/Packing/Freight cards — the freight⇐packing dependency presented, not "fixed") + freight "per unit"→"Rate / container" relabel + empty selects "—"→"Select…". **Files:** `OperationsTab.tsx`, `lib/order-pills.ts`, `lib/action-center.ts`, `components/action-center/ActionCenter.tsx`, `projects/[id]/page.tsx`, `projects/[id]/FreightEntryForm.tsx`. **Verified** (genuine Operations login): merged KPI section renders (both groups, no old strip); chips read "Now due / 4d late / 24d late" (no "+Xd"); O1 sequence intro + numbered cards on an "Operations in progress" SR, relabels on a priced SR. `tsc` (no new errors) + `check:schema` ✅ + `npm test` 252/252 + `e2e:regression` 23/23. Committed.
